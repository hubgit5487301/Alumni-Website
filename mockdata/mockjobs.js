...
const formats = ['21CSE', '21ME', '21ECE', '21EE', '21CE'];
...
async function generateMockData() {
  ...
  const mockJobs = Array.from({ length: 50 }, () => ({
    userid: '248650',
  ...
  for (const job of mockJobs) {
    const numApplicants = faker.number.int({ min: 1, max: 10 });  // Random number of applicants (1 to 10)

    const applicants = Array.from({ length: numApplicants }, () => {
      const randomFormat = faker.helpers.arrayElement(formats);
      const randomNumber = faker.number.int({ min: 1, max: 100 }).toString().padStart(2, '0'); // Random number (01 to 100)
      return { applicant: `${randomFormat}${randomNumber}` };
    });
